...
def get_data_type():
	...
	if len(sys.argv) == 1 or sys.argv[1] in ['-h', '--help']:
		...
		print('')
		print('description: identify core-genome snps from <module>')
		print('')
		print('modules:')
		...
		print('')
		quit()
	elif sys.argv[1] not in ['genomes', 'msa', 'db', 'single', 'reads', 'genotype']:
...
def parse_args():
	...
	if data_type in ['genomes', 'genomes-alt', 'single']:
	...
	elif data_type in ['single', 'reads']:
		io.add_argument('--vcf', type=str, metavar='PATH', required=True,
			help = """Path to core-genome snps called from intra-specific genomes. This option is not compatible with --fna-dir. If both provided, this option overrides --fna-dir""")
		io.add_argument('--rep-fna', type=str, metavar='PATH', required=True,
			help = """Path to the reference genome serving as the template for whole genome alignment or reads mapping. Noted that this reference genome should be the same reference genome used for generating vcf""")

	if data_type in ['genomes', 'genotype']:
	...
	if data_type in ['genomes', 'msa', 'single']:
	...
	if data_type in ['db']:
		...
		db.add_argument('--kmer-type', dest='kmer_type', default='all',
			choices=['all', 'center'],
			help="""Choose type of kmers to be fetched
					all: all elligible kmers 1) covered snp at any position
